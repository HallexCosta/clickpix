...
import { hooks } from '../sdk/hooks/hooks.js'
import { hooksIds } from '../sdk/hooks/hooksIds.js'
import { memoryAdapter } from './adapters/memoryAdapter.js'
import { reactMemoryAdapter } from './adapters/reactMemoryAdapter.js'
import { vanillaMemoryAdapter } from './adapters/vanillaMemoryAdapter.js'
import { generateCorrelationUniqueId } from './common/generateCorrelationUniqueId.js'
import { fetchPixChargeQuery } from './services/fetchPixChargeQuery.js'
import { states } from './states.js'
import { createChargeRender } from './templates/createChargeRender.js'

const openpixStaticInstance = {
  memory: memoryAdapter,
  init() {
    console.log('initialize sdk')
    console.log({ vanillaMemoryAdapter })
    const appID = config.appID

    document.body.innerHTML =
      this._createChargeModal() + document.body.innerHTML
    ...
    // const loadMemory = () => {
    //   setTimeout(() => {
    //     console.log('valor atual da memoria', this.memory.states)
    //     console.log(reactMemoryAdapter)
    //     if (!this.memory.states) {
    //       loadMemory()
    //       return
    //     }
    ...
    console.log(this.memory, openpixStaticInstance.hooks)
  },
  setMemoryAdapter(memoryAdptee) {
    this.memory = memoryAdptee
    return memoryAdptee
  },
  _loadMemory(elements) {
    console.log('> _loadMemory', this.memory, this.hooks)
    Array.from(elements, (htmlClickPix) => {
      ...
      if (!this.memory.states.products.has(productId)) {
        this.memory.states.products.set(productId, {
        ...
        console.log(this._displayCheckoutOrPixChargeModal)
        htmlClickPix.onclick = this._displayCheckoutOrPixChargeModal.bind(
          this,
          productId
        )
      } else {
        ...
        htmlClickPix.classList.add('ring-2')
        htmlClickPix.classList.add('ring-red-500')
        htmlClickPix.classList.add('ring-offset-0.5"')
        console.error('[data-product-id] cannot have a duplicate value')
      }
    })
  },
  _displayCheckoutOrPixChargeModal(productId) {
    console.log({ displayHooks: this.hooks })
    this._reRenderChargeModal(productId)

    const product = this._getState('products').get(productId) ?? null
    if (product.status === 'CHARGE_EMITTED') {
      document
        .querySelector(
          `.modal#charge-render[data-product-id="${product.productId}"]`
        )
        .classList.add('open')

      return console.log('render charge using order')
    }

    const modal = document.querySelector('.clickpix-checkout')
    const checkoutForm = modal.querySelector('form')
    ...
    const createOrderHandler = (e) => {
      ...
      // console.log(valid)
      // if (!valid) return console.log('validation failed')

      this._createOrder.apply(this, [e, productId])
    }
    this._configureCloseClickPixCheckoutModal()

    modal.classList.add('open') // vanilla
    // window.openCheckout() // react

    console.log(checkoutForm)
    // modal.querySelector('#charge-continue').onclick = createOrderHandler
    const checkoutButton = checkoutForm.querySelector('button')

    console.log(checkoutButton)
    checkoutButton.onclick = createOrderHandler
    return
  },
  async _createOrder(e, productId) {
    const correlationID = generateCorrelationUniqueId(30)

    const checkoutForm = new FormData(
      document.querySelector('.clickpix-checkout.modal form')
    )
    console.log(this._getState('products'))
    const productsState = this._getState('products')
    productsState.set(
      productId,
      Object.assign(productsState.get(productId), {
        email: checkoutForm.get('email'),
        name: checkoutForm.get('name'),
        document: checkoutForm.get('document'),
        phone: checkoutForm.get('phone'),
        correlationID
      })
    )

    console.log('before calling hook data', productsState.get(productId))

    this._btnLoading.enable(
      document.querySelector('.clickpix-checkout #charge-continue')
    )
    ...
    if (beforeCreateChargeFn) {
      ...
      if (
        hookId !== hooksIds.get('beforeCreateCharge') &&
        typeof hooksIds.get('beforeCreateCharge') === 'string'
      ) {
        this._btnLoading.disable(
          document.querySelector('.clickpix-checkout #charge-continue')
        )
        return console.error('abort create charge event')
      }
    }

    this._createCharge.call(this, productId)
  },
  _createCharge(productId) {
    const memoryOrder = this._getState('products').get(productId)
    console.log({ memoryOrder })

    const additionalInfo = memoryOrder.additionalInfo
      .split(',')
      .reduce((acc, curr) => {
    ...
    const chargeBody = {
      value: Number(memoryOrder.value),
      correlationID: memoryOrder.correlationID,
      customer: {
        email: memoryOrder.email,
        document: memoryOrder.document
      },
      comment: memoryOrder.comment,
      additionalInfo
    }
    console.log({ chargeBody })

    console.log('charge pix woovi called')
    window.$openpix.push(['pix', chargeBody])

    // window.$openpixSDK.updateIn(productId, 'products', {
    //   ...window.$openpixSDK.get(productId, 'products'),
    //   status: 'CHARGE_EMITTED'
    // })
    this._getState('products').set(
      productId,
      Object.assign({
        ...this._getState('products').get(productId),
        status: 'CHARGE_EMITTED'
      })
    )
    ...
    const syncPaymentLink = () => {
      const memorySyncPaymentLinkTimeoutId = setTimeout(() => {
        console.log({ correlationID: memoryOrder.correlationID })
        fetchPixChargeQuery(memoryOrder.correlationID)
          .then(({ data }) => {
            ...
            document.querySelector('#charges-render').insertAdjacentHTML(
              'beforebegin',
              createChargeRender({
                paymentLinkUrl,
                productId
  ...
  _reRenderChargeModal(productId) {
    console.log(productId)
    const productOrder = this._getState('products').get(productId)
    document.querySelector('.modal.clickpix-checkout #value').innerHTML =
      this._centsToBRL(productOrder.value)
  },
  // formatter function
  _centsToBRL(cents) {
            ...
            <form class="modal-form">
              <div class="form-group">
                <input
                  ...
                  minlength="11"
                  maxlength="14"
                  data-pristine-required-message="CPF/CNPJ é obrigatório"
                  data-pristine-minlength-message="Deve ter pelomenos 11 digitos"
                  data-pristine-maxlength-message="Deve ter no maximo 14 digitos"
                />  
              </div>
              <div class="form-group">
                <input
                  ...
                  maxlength="50"
                  required
                  data-pristine-required-message="É necessário digitar um nome"
                  data-pristine-minlength-message="Deve ter pelomenos 4 digitos"
                  data-pristine-maxlength-message="Deve ter pelomenos 50 digitos"
                />
              </div>
              <div class="form-group">
                <input
                  ...
                  class="modal-input"
                  required
                  data-pristine-required-message="Email é obrigatorio"
                  data-pristine-email-message="Digite um email valido"
                />
              </div>
              <div class="form-group">
                <span class="modal-phone-code">+55</span>
                <input name="phone" type="tel" placeholder="Celular" class="modal-input" />
              </div>
              <button id="charge-continue" type="button" class="modal-button">Continuar</button>
            </form>
          </div>
        </div>
      </div>
  ...
  _getStates() {
    return this.memory.getStates()
  },
  _getState(state) {
    console.log('> _getState')
    console.log({ memory: this.memory })
    return this.memory.getStates()[state]
  }
}

const createOpenPixInstance = (memoryAdaptee) => {
  console.log({ openpixStaticInstanceCreated })

  // if (openpixStaticInstanceCreated) {
  //   vanillaMemoryAdapter.setStates(states)
  //   openpixStaticInstance.setMemoryAdapter(memoryAdaptee)
  //   return openpixStaticInstance
  // }
  window.openpixStaticInstanceCreated = true
  // if (openpixStaticInstance.hooks && openpixStaticInstance.hooks.size === 0) {
  //   openpixStaticInstance.hooks = hooks
  // }
  if (!openpixStaticInstance.hooks) {
    openpixStaticInstance.hooks = hooks
  }
  openpixStaticInstance.setMemoryAdapter(memoryAdaptee)
  // openpixStaticInstance.memory.setStates(states)
  return openpixStaticInstance
}

vanillaMemoryAdapter.setStates(states)

export const openpix = createOpenPixInstance(vanillaMemoryAdapter)
